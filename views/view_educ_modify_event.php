...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto p-8">
            ...
            <?php else: ?>
                <?php foreach ($currentEvents as $event): ?>
                    <div class="bg-white shadow-md p-4 rounded-lg">
                        <h3 id="title_<?= isset($event['idEvent']) ? htmlspecialchars($event['idEvent']) : '' ?>"
                            class="text-lg font-semibold mb-2">
                            <?= isset($event['titre']) ? htmlspecialchars($event['titre']) : '' ?>
                        </h3>
                        <p id="description_<?= isset($event['idEvent']) ? htmlspecialchars($event['idEvent']) : '' ?>"
                            class="text-gray-600 mb-4">
                            <?= isset($event['description']) ? htmlspecialchars($event['description']) : '' ?>
                        </p>
                        <p class="text-gray-500">Date de début :
                            <?= isset($event['dateDebut']) ? htmlspecialchars($event['dateDebut']) : '' ?>
                        </p>
                        <p class="text-gray-500">Date de fin :
                            <?= isset($event['dateFin']) ? htmlspecialchars($event['dateFin']) : '' ?>
                        </p>
                        <a href="#"
                            onclick="showModifyForm('<?= isset($event['idEvent']) ? htmlspecialchars($event['idEvent']) : '' ?>'); return false;"
                            class="text-blue-500 hover:text-blue-700 font-semibold block mt-2">Modifier</a>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
        </div>
        ...
        <div class="flex justify-center items-center mt-8">
            <form action="controller_educ_modify_event.php" method="POST" id="modifyEventForm" enctype="multipart/form-data"
                ...
                <input type="datetime-local" name="newStartDate" id="newStartDate"
    ...
    <script>
        function showModifyForm(eventId) {
            ...
            var description = document.getElementById('description_' + eventId).innerHTML;
